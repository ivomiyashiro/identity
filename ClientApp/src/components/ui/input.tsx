  ...
  return (
    ...
    >
      ...
      <input
        ...
        data-slot="input"
        {...props}
        className="file:text-foreground placeholder:text-muted-foreground selection:text-primary-foreground flex h-8 w-full min-w-0 text-base shadow-xs transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm"
      />

      {actionIcon && action && (
        <div className={cn(" flex items-center", actionPositionClass)}>
          <Button onClick={action} variant={actionVariant} type="button" size="icon">
            {actionIcon}
          </Button>
        </div>
      )}
